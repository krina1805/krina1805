...
const CustomerServiceChat = () => {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      try {
        const response = await axios.get(`${API}/chat/history/${sessionId}/customer_service`);
        if (response.data.messages && response.data.messages.length > 0) {
          setMessages(response.data.messages);
        } else {
          setMessages([{
            role: 'assistant',
            content: 'Welcome to customer service! I can help you with product inquiries, service information, returns, troubleshooting, and answer common questions. What can I assist you with today?',
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: '#5C6661' }}>Product & service assistance</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto px-6 py-6">
          <div className="flex gap-4">
            <input
              ...
              placeholder="Ask about products, services, returns..."
